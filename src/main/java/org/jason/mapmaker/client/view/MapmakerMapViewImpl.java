...
public class MapmakerMapViewImpl extends ViewWithUiHandlers<MapPanelUiHandlers>
    ...
    public native void initMap(JavaScriptObject mapOptions, JavaScriptObject bounds, JavaScriptObject border, JsArray markerArray, Element e) /*-{
        ...
        $wnd.getLocationDescription =function(map, lng, lat) {
            that.@org.jason.mapmaker.client.view.MapmakerMapViewImpl::getLocationDescription(Lcom/google/gwt/core/client/JavaScriptObject;DD)(map, lng, lat);
        };

        $wnd.google.maps.event.addListener(map, 'click', function(event) {
            var lat = event.latLng.lat();
            var lng = event.latLng.lng();
            $wnd.getLocationDescription(map, lng, lat);
        });
    }-*/;
    ...
    @Override
    public void getLocationDescription(JavaScriptObject map, double lng, double lat) {
        GWT.log("Executing MapmakerMapViewImpl.getLocationDescription()");
        getUiHandlers().doGetLocationDescription(map, lng, lat);
    }
